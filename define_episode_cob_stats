...
# LA MSA code
LA_MSA_CODE = '4472'  # Los Angeles-Long Beach-Anaheim, CA
...
print(f"LA MSA Code: {LA_MSA_CODE}")
print("="*100)
...
def process_patient_chunk(args):
    ...
    Returns both overall expenditures and LA-specific expenditures.
    ...
    try:
        ...
        # Load data - INCLUDING MSA for geographic filtering
        chunk_df = conn.execute(f"""
            SELECT ENROLID, SVCDATE, DX1, DX2, DX3, DX4, COB, MSA
            FROM '{file_path}'
        ...
        if len(chunk_df) == 0:
            return np.array([]), np.array([]), chunk_idx
        ...
        chunk_df['MSA'] = chunk_df['MSA'].astype(str)
        chunk_df = chunk_df.dropna(subset=['SVCDATE'])
        
        # Define episodes
        chunk_df = define_episodes_optimized(chunk_df, DX_DIGITS, TIME_WINDOW_DAYS)
        
        # Create episode-level data with MSA
        # For each episode, we'll use the MSA from the first visit in that episode
        episode_msa = chunk_df.groupby(['ENROLID', 'episode_id'], sort=False)['MSA'].first()
        
        # Sum expenditures per episode
        episode_expenditures_grouped = chunk_df.groupby(['ENROLID', 'episode_id'], sort=False)['COB'].sum()
        
        # Overall expenditures
        overall_expenditures = episode_expenditures_grouped.values
        
        # LA-specific expenditures (episodes where first visit has LA MSA)
        la_mask = episode_msa == LA_MSA_CODE
        la_expenditures = episode_expenditures_grouped[la_mask].values
        
        # Clean up
        del chunk_df
        gc.collect()
        
        return overall_expenditures, la_expenditures, chunk_idx
...
def process_year_parallel(year):
    ...
    Returns tuple of (all_expenditures, la_expenditures).
    """
    ...
    with Pool(processes=N_PROCESSES) as pool:
        ...
        for overall_exp, la_exp, chunk_idx in results:
            if len(overall_exp) > 0:
                all_expenditures.extend(overall_exp)
            if len(la_exp) > 0:
                la_expenditures.extend(la_exp)
            ...
            if chunk_idx % 50 == 0:
                ...
                print(f"[{year}] Progress: {chunk_idx}/{total_chunks} | "
                      f"Episodes: {len(all_expenditures):,} (LA: {len(la_expenditures):,}) | "
    ...
    la_expenditures = np.array(la_expenditures)
    ...
    print(f"  LA Episodes: {len(la_expenditures):,} ({len(la_expenditures)/len(all_expenditures)*100:.1f}%)")
    print(f"  Total Expenditure: ${np.sum(all_expenditures):,.2f}")
    print(f"  LA Expenditure: ${np.sum(la_expenditures):,.2f}")
    print(f"  Mean Expenditure (Overall): ${np.mean(all_expenditures):,.2f}")
    print(f"  Mean Expenditure (LA): ${np.mean(la_expenditures):,.2f}")
    print(f"  Final Memory: {final_memory:.2f} GB")
    
    return all_expenditures, la_expenditures

def compute_statistics(expenditures, year, suffix=""):
    """
    ...
    if len(expenditures) == 0:
        print(f"[{year}] WARNING: No expenditures to compute statistics{suffix}")
        return None
    
    print(f"\n[{year}] Computing statistics{suffix} on {len(expenditures):,} episodes...")
    ...
    # Save to CSV
    stats_file = os.path.join(output_dir, f'expenditure_statistics_{year}{suffix}.csv')
    stats_df.to_csv(stats_file, index=False)
    
    # Print summary
    print(f"\n[{year}] EXPENDITURE STATISTICS{suffix.upper()}:")
...
def create_histograms(expenditures, la_expenditures, year):
    """
    Create the three requested histograms (Task 3).
    ...
    C) Same thing, showing only data from Los Angeles MSA
    """
    if len(expenditures) == 0:
    ...
    axes[0].axvline(np.median(filtered_a), color='red', linestyle='--', linewidth=2, label=f'Median: ${np.median(filtered_a):,.2f}')
    axes[0].legend()
    ...
    axes[1].set_title(f'{year}: Lower Tail - Overall (0-10th percentile)\nn={len(filtered_b):,} episodes', 
                     fontsize=12, fontweight='bold')
    ...
    # C) LA MSA - Lower tail (0th to 10th percentile)
    if len(la_expenditures) > 0:
        p10_la = np.percentile(la_expenditures, 10)
        filtered_c = la_expenditures[la_expenditures <= p10_la]
        axes[2].hist(filtered_c, bins=50, edgecolor='black', alpha=0.7, color='green')
        axes[2].set_title(f'{year}: Lower Tail - LA MSA (0-10th percentile)\nn={len(filtered_c):,} episodes', 
                         fontsize=12, fontweight='bold')
        axes[2].set_xlabel('Expenditure per Episode ($)', fontsize=11)
        axes[2].set_ylabel('Frequency', fontsize=11)
        
        # Check for clustering in LA
        min_val_la = np.min(filtered_c)
        near_min_la = np.sum((filtered_c >= min_val_la) & (filtered_c <= min_val_la + 10))
        axes[2].axvline(min_val_la, color='red', linestyle='--', linewidth=2, 
                       label=f'Min: ${min_val_la:,.2f}\n{near_min_la:,} episodes within $10')
        axes[2].legend()
        axes[2].grid(True, alpha=0.3)
    else:
        axes[2].text(0.5, 0.5, f'No LA MSA data\nfor {year}', 
                    ha='center', va='center', transform=axes[2].transAxes, 
                    fontsize=14, bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5))
        axes[2].set_title(f'{year}: LA MSA (0-10th percentile)', fontsize=12, fontweight='bold')
    ...
    print(f"\n[{year}] CLUSTERING ANALYSIS (Lower Tail - Overall):")
    ...
    if len(la_expenditures) > 0:
        print(f"\n[{year}] CLUSTERING ANALYSIS (Lower Tail - LA MSA):")
        print(f"  Minimum value: ${min_val_la:,.2f}")
        print(f"  Episodes within $10 of min: {near_min_la:,} ({near_min_la/len(filtered_c)*100:.1f}% of LA lower tail)")
        print(f"  Episodes within $50 of min: {np.sum((filtered_c >= min_val_la) & (filtered_c <= min_val_la + 50)):,}")
        print(f"  Episodes within $100 of min: {np.sum((filtered_c >= min_val_la) & (filtered_c <= min_val_la + 100)):,}")

def save_expenditure_data(expenditures, la_expenditures, year):
    """
    Save the raw expenditure data for potential further analysis.
    """
    print(f"\n[{year}] Saving raw expenditure data...")
    
    # Save overall expenditures
    ...
    print(f"  Overall data saved to: {expenditure_file} ({file_size:.1f} MB)")
    
    # Save LA expenditures
    if len(la_expenditures) > 0:
        la_file = os.path.join(output_dir, f'expenditures_{year}_LA.npy')
        np.save(la_file, la_expenditures)
        la_file_size = os.path.getsize(la_file) / (1024**2)
        print(f"  LA data saved to: {la_file} ({la_file_size:.1f} MB)")
...
if __name__ == '__main__':
    ...
    for year in YEARS:
        ...
        try:
            ...
            expenditures, la_expenditures = process_year_parallel(year)
            
            if expenditures is None or len(expenditures) == 0:
                print(f"\n[{year}] ✗ No data processed")
                continue
            
            # Task 1 & 2: Compute statistics for overall data
            stats = compute_statistics(expenditures, year, suffix="")
            if stats is not None:
                all_year_stats.append(stats)
            
            # Task 1 & 2: Compute statistics for LA data
            if len(la_expenditures) > 0:
                la_stats = compute_statistics(la_expenditures, year, suffix="_LA")
                if la_stats is not None:
                    all_year_la_stats.append(la_stats)
            
            # Task 3: Create histograms
            create_histograms(expenditures, la_expenditures, year)
            
            # Save raw data
            exp_file = save_expenditure_data(expenditures, la_expenditures, year)
            expenditure_files[year] = exp_file
            
            print(f"\n[{year}] ✓ Year complete")
            
            # Clean up to free memory for next year
            del expenditures, la_expenditures
            gc.collect()
    ...
    if len(all_year_stats) > 0:
        ...
        print(f"\n✓ Combined overall statistics saved to: {combined_file}")
        
        # LA statistics
        if len(all_year_la_stats) > 0:
            combined_la_stats = pd.concat(all_year_la_stats, ignore_index=True)
            combined_la_file = os.path.join(output_dir, 'expenditure_statistics_all_years_LA.csv')
            combined_la_stats.to_csv(combined_la_file, index=False)
            print(f"✓ Combined LA statistics saved to: {combined_la_file}")
        
        # Print summary tables
        print("\n" + "="*100)
        print("SUMMARY TABLE - OVERALL (Minimum Statistics)")
        print("="*100)
        ...
        print("\n" + "="*100)
        print("SUMMARY TABLE - OVERALL (Deciles)")
        print("="*100)
        ...
        if len(all_year_la_stats) > 0:
            print("\n" + "="*100)
            print("SUMMARY TABLE - LA MSA (Minimum Statistics)")
            print("="*100)
            min_stats_la = combined_la_stats[combined_la_stats['statistic'].isin(['minimum_overall', 'minimum_p1', 'minimum_p5'])]
            pivot_la = min_stats_la.pivot(index='year', columns='statistic', values='value')
            print(pivot_la.to_string())
            
            print("\n" + "="*100)
            print("SUMMARY TABLE - LA MSA (Deciles)")
            print("="*100)
            decile_stats_la = combined_la_stats[combined_la_stats['statistic'].str.match(r'^p\d0$')]
            decile_pivot_la = decile_stats_la.pivot(index='year', columns='statistic', values='value')
            decile_pivot_la = decile_pivot_la[[c for c in col_order if c in decile_pivot_la.columns]]
            print(decile_pivot_la.to_string())
    ...
    print(f"\nFiles generated:")
    print(f"  - expenditure_statistics_YYYY.csv (per year - overall)")
    print(f"  - expenditure_statistics_YYYY_LA.csv (per year - LA MSA)")
    print(f"  - expenditure_statistics_all_years.csv (combined - overall)")
    print(f"  - expenditure_statistics_all_years_LA.csv (combined - LA MSA)")
    print(f"  - expenditure_histograms_YYYY.png (per year with all 3 panels)")
    print(f"  - expenditures_YYYY.npy (raw overall data per year)")
    print(f"  - expenditures_YYYY_LA.npy (raw LA data per year)")
    print("="*100)
